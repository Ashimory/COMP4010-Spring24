...
syr_plot_labelled <- syr_plot_colored +
  ...
  geom_text(
    data = aqi_levels,
    aes(x = ymd("2024-03-01"), y = aqi_mid, label = level),
...
freedom_faceted_plot <- freedom_ranked |>
  ...
  ggplot(aes(x = civil_liberty, y = country, fill = is_venezuela)) +
  ...
  geom_text(
    hjust = "left", #check back
    aes(label = country), #check back
    x = -5
  ) +
...
freedom_bar_race <- freedom_faceted_plot +
  ...
  labs(
    title = "Civil liberties rating, {as.integer(frame_time)}",
